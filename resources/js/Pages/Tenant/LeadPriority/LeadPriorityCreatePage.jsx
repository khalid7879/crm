...
export default function LeadPriorityCreatePage() {
    ...
    const handleLeadPrioritySubmit = (e) => {
        e.preventDefault();
        clearErrors();
        ...
        post(route(routeNames.leadPrioritiesStore, { tenant }), data, {
            preserveScroll: true,
            onSuccess: () => {
    ...
    return (
        <TenantSettingLayout metaTitle={metaTitle}>
            <Breadcrumb
                title={metaTitle}
                btnIcon="list"
                navItems={[...LeadPriorityListNavItems, { name: "Create" }]}
            ...
            <TableCardComponent>
                <form
                    id=""
                    className="space-y-6"
                    onSubmit={handleLeadPrioritySubmit}
                    noValidate
                >
                    {/* Single Input Field */}
