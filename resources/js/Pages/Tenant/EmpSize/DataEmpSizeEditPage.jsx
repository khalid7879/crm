...
import { TenantEmpSizeSchema } from "@/schemas/tenants/tenantEmpSizeSchema";
import { TenantDataEmpSizeCreateInputs } from "@/utils/forms/dataEmpSizeInputs";
import { DataEmpSizeListNavItems } from "@/utils/common/BreadcrumbNavItems";
...
export default function DataEmpSizeEditPage() {
    const route = useRoute();
    const __ = useTranslations();
    const metaTitle = __("Employee size");
    const { tenant, routeNames, dataEmpSizes } = usePage().props;
    const {
        data,
        setData,
        post,
        ...
        name: dataEmpSizes?.name || "",
        note: dataEmpSizes?.note || "",
        size: dataEmpSizes?.size || "",
    });
    const { toastAlert } = usePage().props;

    const schema = useMemo(() => TenantEmpSizeSchema(__), []);
    ...
    const handleModelSubmit = (e) => {
        ...
        put(
            route(routeNames.dataEmpSizesUpdate, {
                tenant,
                emp_size: dataEmpSizes.id,
            }),
            data,
            {
                preserveScroll: true,
    ...
    return (
        <TenantSettingLayout metaTitle={metaTitle}>
            <Breadcrumb
                title={metaTitle}
                navItems={[...DataEmpSizeListNavItems, { name: "Edit" }]}
                btnIcon="list"
            ...
            <TableCardComponent>
                ...
                >
                    {/* Single Input Field */}
                    {TenantDataEmpSizeCreateInputs.map((field) => (
