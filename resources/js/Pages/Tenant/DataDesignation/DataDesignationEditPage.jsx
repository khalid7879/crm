...
import { TenantDataDesignationSchema } from "@/schemas/tenants/tenantDataDesignationSchema";
import { TenantDataDesignationCreateInputs } from "@/utils/forms/dataDesignationInputs";
...
export default function DataDesignationEditPage() {
    const route = useRoute();
    const __ = useTranslations();
    const metaTitle = __("Designations");
    const { tenant, routeNames, designations } = usePage().props;
    const {
        data,
        setData,
        post,
    ...
    const schema = useMemo(() => TenantDataDesignationSchema(__), []);
    ...
    const handleResourceUpdate = (e) => {
        ...
        put(
            route(routeNames.dataDesignationsUpdate, {
                tenant,
                data_designation: designations.id,
            }),
            data,
            {
                preserveScroll: true,
    ...
    return (
        <TenantSettingLayout metaTitle={metaTitle}>
            <Breadcrumb
                title={metaTitle}
                navItems={[...DesignationListNavItems, { name: "Edit" }]}
                btnIcon="list"
            ...
            <TableCardComponent>
                ...
                >
                    {/* Single Input Field */}
                    {TenantDataDesignationCreateInputs.map((field) => (
