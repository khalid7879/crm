...
export default function DataContactTimeEditPage() {
    const route = useRoute();
    const __ = useTranslations();
    const metaTitle = __("Contact times");
    const { tenant, routeNames, contactTimes } = usePage().props;
    const {
        data,
        setData,
        post,
    ...
    const handleModuleUpdate = (e) => {
        ...
        put(
            route(routeNames.dataContactTimesUpdate, {
                tenant,
                contact_time: contactTimes.id,
            }),
            data,
            {
                preserveScroll: true,
    ...
    return (
        <TenantSettingLayout metaTitle={metaTitle}>
            <Breadcrumb
                title={metaTitle}
                navItems={[...ContactTimeListNavItems, { name: "Edit" }]}
                btnIcon="list"
