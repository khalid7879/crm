...
/**
 * @component
 * DataContactTimeListPage
 *
 * @description
 * This component displays the list view for managing Contact Times within the tenant settings.
 * It provides a filterable, sortable, and paginated table showing all contact time records.
 * Users can:
 * - Search and filter the list by per-page count, ordering, and active status
 * - Toggle the active/inactive status of a contact time
 * - Edit or delete individual contact time entries
 * - Navigate to the create new contact time page
 * - View creation and update timestamps
 *
 * The component uses Inertia.js for server-side rendering and form handling,
 * integrates with custom table and pagination components, and shows toast notifications
 * for success/error messages passed via page props.
 *
 * @author Sakil Jomadder <sakil.diu.cse@gmail.com>
 * @author Mamun <mamunhossen149191@gmail.com>
 */
export default function DataContactTimeListPage() {
    const route = useRoute();
    const __ = useTranslations();
    const metaTitle = __("Contact times");

    const { tenant, routeNames, contactTimeList, filterOptions, toastAlert } =
        usePage().props;
        ...
        (id) => {
            swalAlert({
                ...
                if (result.isConfirmed) {
                    clearErrors();
                    router.delete(
                        route(routeNames.dataContactTimesDelete, {
                            tenant,
                            contact_time: id,
                        }),
                        {
                            ...
                            onSuccess: () => {
                                console.log(
                                    "Contact time deleted successfully"
                                );
                            },
                            onError: (errors) => {
    ...
    const handleModelStatusChange = useCallback(
        (id) => {
            post(
                route(routeNames.dataContactTimesStatusChange, {
                    tenant,
                    contactTimeId: id,
                }),
                {
    ...
    const action = useMemo(
        () => [
            {
                name: "EDIT",
                link: routeNames.dataContactTimesEdit,
                params: "contact_time",
            },
            {
                name: "DELETE",
    ...
    const handleSubmit = useCallback(
        (e) => {
            e.preventDefault();
            get(route(routeNames.dataContactTimesList, { tenant }), {
                preserveScroll: true,
            });
        },
    ...
    return (
        <TenantSettingLayout metaTitle={metaTitle}>
            <Breadcrumb
                title={metaTitle}
                navItems={[...ContactTimeListNavItems, { name: "List" }]}
                btnIcon="add"
                link="tenant.contact-times.create"
            />
            ...
            <TableCardComponent>
                <FilterFormComponent
                    ...
                    resetLink={routeNames.dataContactTimesList}
                />
                    ...
                    <TableBodyComponent>
                        {allModels?.length > 0 ? (
                            allModels.map((model, index) => (
                                <TableTrComponent
                                    key={`contactTime_${model.id ?? index}`}
                                >
                                    <TableTdComponent
                                        label={model?.name}
                                        isDataIcon={true}
