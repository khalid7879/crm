...
import { useProjectNoteFormInputs } from "@/hooks/useProjectNoteFormInputs";
 ...
 */
export default function ProjectNoteModal({
    isModalOpen,
    setIsModalOpen,
    finalStepInfos,
    modalTitle,
    ...
    const stateDefaults = useMemo(() => ({
        ...
        progress_percent:
            dataStagesForProject?.list?.[
                dataStagesForProject?.default
            ]?.stage_percent,
    ...
    const handleSubmit = useCallback(
        (e, isSaveAndNew = false) => {
            ...
            post(route(routeNames.projectsChangeStage, { tenant }), {
                preserveScroll: true,
                onSuccess: () => {
                    if (!isSaveAndNew) {
     ...
     * @type {Array}
     */
    const formInputs = useProjectNoteFormInputs({
        data,
        setData,
        errors,
