 ...
 */
export default function ContactDeleteModal({ isModalOpen, setIsModalOpen, currentActionId }) {
    ...
    /**
     * Precompute the route to fetch contact details.
     * @type {string | null}
     */
    const showRoute = useMemo(
        () =>
            currentActionId
                ? route(routeNames.contactsShow, { contact: currentActionId, tenant })
                : null,
        [currentActionId, routeNames, tenant, route]
    );

    /**
     * Precompute the route to delete the contact.
     * @type {string | null}
     */
    const deleteRoute = useMemo(
        () =>
            currentActionId
                ? route(routeNames.contactsDelete, {
    ...
    return (
        <ModalFormInputsLayout
            title={__("Delete contact text")}
            isModalOpen={isModalOpen}
            setIsModalOpen={setIsModalOpen}
            size="md"
            processing={processing}
            headerIcon="delete"
            saveText={__("Yes delete the contact text")}
        ...
        >
            <ul className="list bg-base-100 rounded-box shadow-md divide-y divide-gray-200">
                ...
                <li className="py-2 px-1 text-[14px] font-semibold text-red-600 tracking-wide">
                    {__("Are you sure to delete this contact")}
                </li>
