...
import { useOpportunityNoteFormInputs } from "@/hooks/useOpportunityNoteFormInputs";
 ...
 */
export default function OpportunityNoteModal({
    isModalOpen,
    setIsModalOpen,
    finalStepInfos,
    modalTitle,
    ...
    const stateDefaults = useMemo(() => ({
        ...
        is_active: true,
        stageable_id: finalStepInfos.stageable_id,
        stage_id: finalStepInfos.stage_id,
        finalStepInfos,
    }));
    ...
    const userOptions = useMemo(() =>
        Object.entries(model.tenantUsers?.list || {}).map(([id, name]) => ({
            value: id,
            label: name,
        }))
    );
    ...
    const handleSubmit = useCallback(
        (e, isSaveAndNew = false) => {
            ...
            post(route(routeNames.opportunityChangeStage, { tenant }), {
                preserveScroll: true,
                onSuccess: () => {
                    if (!isSaveAndNew) {
                ...
                onError: (errors) => {
                    ...
                    // Example: show first validation message
     ...
     * @type {Array}
     */
    const formInputs = useOpportunityNoteFormInputs({
        data,
        setData,
        errors,
